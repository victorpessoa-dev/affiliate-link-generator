...
function ensureExpectedAffiliateUrl({ apiUrl, resolvedUrl, platform, expectedId, }) {
    if (apiUrl) {
        const apiHasAffiliate = hasAffiliateParams(apiUrl, platform);
        const apiAffiliateId = extractAffiliateId(apiUrl, platform);
        if (apiHasAffiliate && apiAffiliateId === expectedId) {
            return {
                url: apiUrl,
                method: "api",
                source: "api-validado",
        ...
        logAffiliateConversion("warn", "API retornou link sem o ID esperado; usando fallback local", {
            platform,
            expectedId,
            foundId: apiAffiliateId,
            hasAffiliateParams: apiHasAffiliate,
        });
    }
    return {
...
function readAffiliateUrl(data, keys) {
...
async function extractRenderedPreview(url) {
    if (process.env.PREVIEW_FALLBACK === "false")
        return null;
    const endpoint = process.env.PREVIEW_API_URL || "https://api.microlink.io/";
    try {
        ...
        if (!data)
            return null;
        const rawImage = readNestedUrl(data.image) || readNestedUrl(data.screenshot);
        let image = null;
        if (rawImage) {
            try {
...
async function extractProductMetadata(url) {
    ...
    const metadata = partialMetadata?.image
        ? partialMetadata
        : mergeMetadata(partialMetadata, await extractRenderedPreview(url));
...
async function convertShopeeViaApi(url) {
...
async function convertMercadoLivreViaApi(url) {
...
export async function convertAffiliateLink(rawUrl) {
    ...
    const affiliate = ensureExpectedAffiliateUrl({
        apiUrl: affiliateUrl,
        resolvedUrl,
        platform,
        expectedId,
